...
The primary entry point for executing the pipeline is the `run.sh` script.
...
To execute the pipeline, run the `run.sh` script and pass the number of GPUs you want to use for the training phase.
...
The `run.sh` script performs the following automated steps safely within a `tmux` session named `pla_pipeline`:
